...
public final class PropertiesLoader {
	...
	public static Properties load() throws IOException {
		...
		if (testDefaultProperties != null) {
		    ...
		    if (fallbackEnv != null) {
		        System.setProperty("env", fallbackEnv.trim());
		    }
	    ...
	    if (isNonBlank(urlSpec)) {
	        ...
	        try {
	            return CACHE.computeIfAbsent(key, k -> {
	                try { return loadFromUrlOrFileSpec(urlSpec); }
	    ...
	    try {
	        return CACHE.computeIfAbsent(defaultCacheKey, k -> {
	            try {
	            	...
	            	Properties mainCachedDeafultProperties = tryLoadFromClasspath(MAIN_DEFAULT_ENV_FILE);
	            	if (mainCachedDeafultProperties != null) {
	            	    String fallbackEnv = mainCachedDeafultProperties.getProperty("env");
	            	    if (fallbackEnv != null) {
	            	        System.setProperty("env", fallbackEnv.trim());
	            	    }
	            	    return mainCachedDeafultProperties;
	            	}

	                throw new FileNotFoundException(
	                    "Environment not supplied and default configs not found. " +
